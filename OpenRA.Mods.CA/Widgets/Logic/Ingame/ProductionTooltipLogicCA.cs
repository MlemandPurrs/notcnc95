...
namespace OpenRA.Mods.CA.Widgets.Logic
{
	public class ProductionTooltipLogicCA : ChromeLogic
	{
		...
		public ProductionTooltipLogicCA(Widget widget, TooltipContainerWidget tooltipContainer, Player player, Func<ProductionIcon> getTooltipIcon)
		{
			...
			var canattackLabel = widget.Get<LabelWidget>("CANATTACK");
			var upgradesLabel = widget.Get<LabelWidget>("UPGRADES");
			var attributesLabel = widget.Get<LabelWidget>("ATTRIBUTES");
			var specialabilitiesLabel = widget.Get<LabelWidget>("SPECIALABILITIES");
			...
			tooltipContainer.BeforeRender = () =>
			{
				...
				if (tooltipExtras != null)
				{
					...
					canattackLabel.Text = tooltipExtras.Canattack.Replace("\\n", "\n");
					upgradesLabel.Text = tooltipExtras.Upgrades.Replace("\\n", "\n");
					attributesLabel.Text = tooltipExtras.Attributes.Replace("\\n", "\n");
					specialabilitiesLabel.Text = tooltipExtras.Specialabilities.Replace("\\n", "\n");
				}
				else
				{
					strengthsLabel.Text = "";
				...
				var canattackSize = canattackLabel.Text != "" ? descFont.Measure(canattackLabel.Text) : new int2(0, 0);
				var upgradesSize = upgradesLabel.Text != "" ? descFont.Measure(upgradesLabel.Text) : new int2(0, 0);
				var attributesSize = attributesLabel.Text != "" ? descFont.Measure(attributesLabel.Text) : new int2(0, 0);
				var specialabilitiesSize = specialabilitiesLabel.Text != "" ? descFont.Measure(specialabilitiesLabel.Text) : new int2(0, 0);
				...
				canattackLabel.Bounds.Y = descLabel.Bounds.Bottom + strengthsSize.Y + weaknessesSize.Y + extrasSpacing;
		...
		LabelWidget GetArmorTypeLabel(LabelWidget armorTypeLabel, ActorInfo actor)
		{
			...
			// hard coded, mod specific - find a better way to set user-friendly names and colors for armor types
			switch (armorTypeLabel.Text)
			{
				case "None":
					armorTypeLabel.Text = "None";
				...
				case "LightInf":
					armorTypeLabel.Text = "Kevlar";
					armorTypeLabel.TextColor = Color.White;
					break;

				case "HeavyInf":
					armorTypeLabel.Text = "Flak";
					armorTypeLabel.TextColor = Color.White;
					break;
